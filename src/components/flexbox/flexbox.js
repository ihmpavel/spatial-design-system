...
AFRAME.registerComponent('flexbox', {
  ...
  applyGrow() {
    this.lines.forEach(line => {
      let freeSpace =
        this.container[this.MAIN_DIMENSION] - this.data.gap[this.MAIN_AXIS] * (line.length - 1)
      ...
      const growItems = line.filter(
        item =>
          item.getAttribute('flex-grow') !== null && item.getAttribute('flex-grow') !== 'false'
      )
      ...
      growItems.forEach(growItem => {
        if (!growItem.hasAttribute(ORIGINAL_DIRECTION_ATTRIBUTE)) {
          growItem.setAttribute(
            ORIGINAL_DIRECTION_ATTRIBUTE,
            growItem.getAttribute(this.MAIN_DIMENSION) || '1'
          )
        }

        growItem.setAttribute(
          this.MAIN_DIMENSION,
          +growItem.getAttribute(ORIGINAL_DIRECTION_ATTRIBUTE) + freeSpacePerItem
        )
  ...
  applyBootstrapGrid() {
    this.lines.forEach(line => {
      ...
      line.forEach(item => {
        ...
        _freeSpace -= itemBboxSize[this.MAIN_AXIS]
      })

      const colItems = line.filter(item => item.getAttribute('flex-col') !== null)
          ...
          line.forEach(item => {
            if (this.isDirectionRow()) {
              ...
              item.object3D.position.y -=
                (maxCrossSizeInLine - this.getItemBboxSize(item)[this.CROSS_AXIS]) / 2
            } else {
        ...
        case ITEMS.END:
          line.forEach(item => {
            if (this.isDirectionRow()) {
              item.object3D.position.y -= freeSpace
              item.object3D.position.y -=
                maxCrossSizeInLine - this.getItemBboxSize(item)[this.CROSS_AXIS]
            } else {
              item.object3D.position.x += freeSpace
              item.object3D.position.x +=
                maxCrossSizeInLine - this.getItemBboxSize(item)[this.CROSS_AXIS]
  ...
  getFreeSpaceForLineItems(line) {
    ...
    for (let i = lineIndex + 1; i < this.lines.length; i++) {
      const maxCrossSizeInNextLine = Math.max(
        ...this.lines[i].map(item => this.getItemBboxSize(item)[this.CROSS_AXIS])
      )
  ...
  getItemBboxSize(item) {
    ...
    return itemBbox.getSize(new AFRAME.THREE.Vector3()).divide(this.el.object3D.scale)
  },
