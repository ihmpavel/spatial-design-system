...
AFRAME.registerComponent('textbox', {
  ...
  createTextbox() {
    ...
    const borderColor =
      this.el.getAttribute('variant') === 'dark' ? VARIANT_LIGHT_COLOR : VARIANT_DARK_COLOR

    const textboxMesh = this.createMesh(
      width,
      height,
      this.el.getAttribute('variant') === 'dark' ? VARIANT_DARK_COLOR : VARIANT_LIGHT_COLOR,
      false
    )
  ...
  switchActivated() {
    ...
    this.el.emit('is-textbox-activated', {
      isActivated: this.data.isactivated,
  ...
  moveTextLine() {
    ...
    if (this.data.moveCursorByAmount + inputEl.value.length < 0) {
      return
    }
    ...
    if (
      this.data.moveCursorByAmount + this.data.numberOfCharactersInTheBox <= 0 &&
    ...
    if (textElContentWidth > maxAllowedTextWidth || this.data.moveWholeTextInBoxByAmount <= 0) {
      this.el.setAttribute('text', {
        ...
        value: inputEl.value.substring(
          inputEl.value.length - maxCharsAmount + this.data.moveWholeTextInBoxByAmount,
  ...
  setEvents() {
    ...
    const setText = _lastKeyboardCharacter => {
      ...
      if (lastPressedKey !== '') {
        ...
        if (lastPressedKey === 'DEL') {
          if (cursorIndex === 0) {
            return // Do nothing if the cursor is at the beginning
          }
          newValue =
          ...
          if (cursorIndex === 0 && inputEl.value.length >= this.data.maxCharsAmount) {
            this.data.moveWholeTextInBoxByAmount -= 1
          }
    ...
    scene.addEventListener('click-nothing', _e => {
      // If the textbox is not activated, or clicking on a object, do nothing
      if (!this.el.object3D.visible || !this.data.isactivated) {
        return
      }
  ...
  updateCursorPosition() {
    ...
    if (cursorX > (this.data.textWidthCoef * 1.05 * this.el.getAttribute('sizeCoef')) / 2) {
      return
    }
    // Or at the beginning of the textbox
    if (cursorX < cursorInitialPositionX) {
      cursorX = cursorInitialPositionX
    }
    if (textEl.value === this.data.label) {
      cursorX = cursorInitialPositionX
    }
