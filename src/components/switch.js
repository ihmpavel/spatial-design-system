...
AFRAME.registerComponent('switch', {
  ...
  updateColors() {
    ...
    this.shadowMesh.material.color.set(
      this.data.value ? this.data.primary : this.data.backgroundcolor
    )
    const circle = this.el.querySelector('a-circle')
  ...
  createSwitch() {
    ...
    const switchMesh = new AFRAME.THREE.Mesh(
      switchGeometry,
      this.data.value ? onMaterial : offMaterial
    )
    ...
    circle.setAttribute(
      'position',
      `${!this.data.value ? -width / (2 * insetCoef + height) / 2 : width / (2 * insetCoef) - height / 2} 0.005 0.01`
    )
    circle.setAttribute('color', this.data.value ? this.data.primary : this.data.circlecolor)
