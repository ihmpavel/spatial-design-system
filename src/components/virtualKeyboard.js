...
import { createRoundedRectShape, createRoundedSquareShape } from '../utils/utils.js'
...
AFRAME.registerComponent('virtualKeyboard', {
  ...
  createKeyboardLayout(layout) {
    ...
    const createKey = (key, _xPos, yPos, size) => {
      ...
      keyEl.setAttribute(
        'position',
        `${this.data.position.x + xPos} ${this.data.position.y + yPos} ${this.data.position.z}`
      )
    ...
    layout.forEach((row, rowIndex) => {
      ...
      rowBackground.setAttribute(
        'position',
        `${this.data.position.x + xPos + rowWidth / 2 - 0.11} ${this.data.position.y + yPos} ${this.data.position.z - 0.01}`
      )
    ...
    if (this.data.isCaps || this.data.isShift) {
      this.changeKeysLowerUpperCase(true)
    }
  },
  ...
  attachKeyListeners(keyElement, keyValue, textElementValue) {
    ...
    keyElement.addEventListener('click', () => {
      if (this.data.keyboard.getAttribute('visible') === false) {
        return
      }
  ...
  deleteLastCharacter() {
    ...
    if (currentText === textInput.getAttribute('label')) {
      return
    }
  ...
  animateKeyVisibility(visible) {
    ...
    this.el.querySelectorAll('.a-ar-key-row-background').forEach(rowBackground => {
      ...
      visible
        ? rowBackground.classList.add('clickable')
        : rowBackground.classList.remove('clickable')
    })
  },
