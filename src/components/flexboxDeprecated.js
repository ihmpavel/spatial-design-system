...
import { center3DModelGeometry, computeBbox, onSceneLoaded } from '../utils/utils.js'
...
AFRAME.registerComponent('flexbox_deprecated', {
  ...
  setGap(x, y) {
    ...
    const isJustifiedSpace =
      this.data.mainAlign === 'space-around' || this.data.mainAlign === 'space-between'

    let gapX = x > 0 && x < 100 ? x / 100 : 0
    let gapY = y > 0 && y < 100 ? y / 100 : 0

    if (isJustifiedSpace) {
      const [containerWidthPerItem, containerHeightPerItem] =
        this.getContainerWidthAndHeightPerItem(isColumnDirection)
  ...
  setItemsLayout() {
    ...
    for (let itemIndex = 0; itemIndex < this.items.length; itemIndex++) {
      ...
      const xScale =
        isStretchAlignment && isColumnDirection
      ...
      this.setItemScale(item, new AFRAME.THREE.Vector3(xScale, yScale, item.object3D.scale.z))

      if (this.isItemWrap(item, isColumnDirection)) {
        const containerDimensionPerItem = isColumnDirection
          ? containerWidthPerItem
          : containerHeightPerItem
        ...
        let containerDimensionWhenStretch = containerHeight
        ...
        const numberOfEmptyCellsPerLine = Math.max(
          Math.floor(containerDimension) - numberOfOccupiedCellsPerLine,
          0
        )
        ...
        const rowOffset = isStretchAlignment
          ? containerDimensionWhenStretch / numberOfWraps
          : containerDimensionPerItem
        rowIndex += rowOffset
  ...
  getContainerWidthAndHeightPerItem(isColumnDirection = false) {
    ...
    if (isWrapSet && itemsExceedContainerDimension) {
      ...
      const containerDimensionFraction =
        containerDimensionWholeNumber > 0
          ? (containerDimension % 1) / containerDimensionWholeNumber
          : 0
      const numberOfWraps = this.getNumberOfWraps(isColumnDirection)
  ...
  getAlignmentOffsets(isColumnDirection = false) {
    ...
    let mainAlignContainerDimension = this.container.width
    let secondaryAlignContainerDimension = this.container.height
    let mainAlignContainerDimensionPerItem = containerWidthPerItem
    let secondaryAlignContainerDimensionPerItem = containerHeightPerItem
    let mainAlignGap = this.normalizedGap.x
    let secondaryAlignGap = this.normalizedGap.y
    ...
    const alignmentValues = {
      mainAlign: {
        ...
        'space-between':
          mainAlignContainerDimensionPerItem +
          (mainAlignContainerDimensionPerItem * mainAlignGap) /
            Math.max(mainAlignContainerDimension / mainAlignContainerDimensionPerItem - 1, 1),
      },
      secondaryAlign: {
        start: 0,
        center:
          secondaryAlignContainerDimension / 2 -
          (secondaryAlignContainerDimensionPerItem / 2) * wrapOffset,
        end:
          secondaryAlignContainerDimension - secondaryAlignContainerDimensionPerItem * wrapOffset,
        stretch:
          secondaryAlignContainerDimension / 2 -
          secondaryAlignContainerDimensionPerItem / 2 -
          (secondaryAlignContainerDimension / 2 / wrapOffset) * (wrapOffset - 1) -
          (secondaryAlignContainerDimension / 2 / wrapOffset) * secondaryAlignGap,
      },
    }

    const mainAlignOffset =
      alignmentValues.mainAlign[mainAlignData] ?? alignmentValues.mainAlign['start']
    const secondaryAlignOffset =
      alignmentValues.secondaryAlign[secondaryAlignData] ?? alignmentValues.secondaryAlign['start']
  ...
  getItemOriginOffsets(isColumnDirection = false) {
    ...
    const offsets = isColumnDirection
      ? [yOffsetFromOrigin, xOffsetFromOrigin]
      : [xOffsetFromOrigin, yOffsetFromOrigin]
