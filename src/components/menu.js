...
AFRAME.registerComponent('menu', {
  ...
  show() {
    ...
    if (this.el.components['fit-into-fov']) {
      // fit-into-fov has priority over auto-scale
      targetScale = this.el.components['fit-into-fov'].calculateNewScale()
    } else if (this.el.components['auto-scale']?.data.enabled) {
  ...
  update(oldData) {
    ...
    document.querySelectorAll('.menu-item a-text').forEach(e => {
    ...
    document.querySelectorAll('.menu-item a-image').forEach(e => {
    ...
    document.querySelectorAll('a-ring').forEach(e => {
  ...
  setItems(parsedItems) {
    ...
    const lastTwoItems = parsedItems.splice(
      parsedItems.length - (parsedItems.length === 4 ? 1 : 2),
      2
    )
    ...
    const nodes = parsedItems.map((parsedItem, index) => {
      ...
      item.addEventListener('click', _e => {
        if (this.data.highlight) {
          ...
          const iconSrc =
            this.selected[index] && !parsedItem.icon.includes('-white')
  ...
  setSupportElements() {
    ...
    if (this.data.backbutton) {
      ...
      arrowback.addEventListener('click', _e => {
...
AFRAME.registerComponent('grid', {
  ...
  initGridProperties() {
    ...
    this.normalizedPadding.x =
      this.data.padding.x > 0 && this.data.padding.x < 100 ? this.data.padding.x / 100 : 0
    this.normalizedPadding.y =
      this.data.padding.y > 0 && this.data.padding.y < 100 ? this.data.padding.y / 100 : 0
    ...
    if (this.gridRows === 1) {
      this.gridColumns =
        this.gridColumns > this.gridItems.length ? this.gridColumns : this.gridItems.length
  ...
  setLayout() {
    ...
    const itemBaseXPosition =
      -this.gridWidth / 2 + itemWidth / 2 - (itemWidth / 2) * this.normalizedPadding.x
    const itemBaseYPosition =
      this.gridHeight / 2 - itemHeight / 2 + (itemHeight / 2) * this.normalizedPadding.y
    const itemXOffset =
      itemWidth + (itemWidth * this.normalizedPadding.x) / Math.max(this.gridColumns - 1, 1)
    const itemYOffset =
      itemHeight + (itemHeight * this.normalizedPadding.y) / Math.max(this.gridRows - 1, 1)
    ...
    for (const item of this.gridItems) {
      ...
      item.object3D.scale.x =
        itemWidth / itemBboxSize.x - (itemWidth / itemBboxSize.x) * this.normalizedPadding.x
      item.object3D.scale.y =
        itemHeight / itemBboxSize.y - (itemHeight / itemBboxSize.y) * this.normalizedPadding.y
...
AFRAME.registerComponent('circle', {
  ...
  updateLayout() {
    ...
    const children = Array.from(this.el.children).filter(
      el => !el.hasAttribute('data-layout-excluded')
    )

    children.forEach((child, index) => {
      const oddItemsLengthAngleSubtraction =
        children.length % 2 === 1 ? (360 / children.length) * (3 / 4) : 0
      ...
      child.setAttribute('geometry', {
        primitive: 'circle',
        radius: circleRadius,
        thetaLength: 360 / children.length,
        thetaStart: thetaStartAngle,
        segments: 64,
      })
      child.setAttribute(
        'position',
        `${circleSpacing * Math.cos(radFromDegrees)} ${circleSpacing * Math.sin(radFromDegrees)} 0.01`
      )

      // text and icon position
      child.children[0]?.setAttribute(
        'position',
        `${0.6 * Math.cos(radFromDegrees) * circleRadius} ${0.6 * Math.sin(radFromDegrees) * circleRadius} 0.01`
      )
    })
  },
})
